...
[vClassHeader("ShooterMelee Input")]
public class vShooterMeleeInput : vMeleeCombatInput
{
    ...
    {       
        ...
        if (!isAttacking)
        {
            ...
            if (!cc.lockMovement && !cc.ragdolled)
            {
                ...
                {
                    isLockingOn = false;
                    if (tpCamera != null) tpCamera.UpdateLockOn(false);
                }
            }
    ...
    protected virtual void UpdateLeftIK()
    {
        ...
        if (leftIK != null)
        {
            ...
            if (shooterManager.rWeapon && shooterManager.rWeapon.handIKTarget && Time.timeScale > 0 && !isReloading && !cc.actions && !cc.customAction && (cc.isGrounded || isAiming) && !cc.lockMovement && useIkConditions)
    ...
    protected virtual void RotateRightArm()
    {
        if (shooterManager.rWeapon && isAiming && aimConditions && shooterManager.rWeapon.alignRightUpperArmToAim)
        {
            ...
            rightRotationWeight = Mathf.Lerp(rightRotationWeight, !shooterManager.shotting || shooterManager.rWeapon.ammoCount <= 0 ? 1f * aimWeight : 0f, 10f * Time.deltaTime);
    ...
    protected virtual void UpdateAimHud()
    {
        ...
        else if (isAiming && aimConditions)
        {
            ...
            {
                controlAimCanvas.SetWordPosition(hit.point);
            }
            else
            {
                controlAimCanvas.SetWordPosition(aimPosition);
            }
        }
        else
